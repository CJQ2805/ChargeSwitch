...
static lv_color_t color_buf1[COLOR_BUF_SIZE]	__attribute__((at(Bank5_SDRAM_ADDR))); //分配到外部SDRAM,需要跳过ltdc.c中分配的帧缓冲区
static lv_color_t color_buf2[COLOR_BUF_SIZE]	__attribute__((at(Bank5_SDRAM_ADDR + COLOR_BUF_SIZE*2)));
...
//    static lv_color_t draw_buf_2_2[LV_HOR_RES_MAX * 10];                        /*An other buffer for 10 rows*/
    lv_disp_buf_init(&draw_buf_dsc_2, color_buf1, color_buf2, COLOR_BUF_SIZE);   /*Initialize the display buffer*/
...
//    }
	...
	//	display_bitmap(area->x1,area->y1,area->x2,area->y2,(u16*)color_p);
    /* IMPORTANT!!!
